// Run "antlr4 -Dlanguage=Python3 -visitor qsrcLexer.g4" if you update this file. You'll need to re-compile the parser as well (see there).
...
TemplateStringEndExpression:
	{self.IsInTemplateString()}? '>>' {self.ProcessTemplateClose();} -> popMode;

DOUBLEQUOTE: '"' -> pushMode(inDQString);
SINGLEQUOTE: '\'' -> pushMode(InString);
TemplateDoubleSingleQuote:{self.IsInTemplateString()}? '\'\'' ->pushMode(InEscapedString);
...
TemplateStringStartExpression:
	'<<' {self.ProcessTemplateOpen();} -> pushMode(DEFAULT_MODE);
